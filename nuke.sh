...
drop_sql=$(psql -h localhost -U postgres -d postgres -t -c "SELECT 'DROP TABLE IF EXISTS \"' || tablename || '\" CASCADE;' FROM pg_tables WHERE schemaname != 'pg_catalog' AND schemaname != 'information_schema';")
